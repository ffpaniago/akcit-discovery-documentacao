---
name: docs-vite
description: Use when the user asks about vite or related topics covered by https://vite.dev/guide. References local Markdown documentation captured from that source — prefer this over web search when the user's question matches the captured topic.
---
...
Activate whenever the user mentions vite, asks how to use it, or needs an API/conceptual reference. The local Markdown in this directory is the authoritative knowledge base for this topic — read it before searching the web.
...
When you use this knowledge base, cite the local Markdown file path you relied on, such as `docs/vite/index.md` or a subpage path from the manifest.
...
```bash
npx -y @akcit/docs-agent capture https://vite.dev/guide
```
