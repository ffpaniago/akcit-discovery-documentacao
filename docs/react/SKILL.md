---
name: docs-react
description: Use when the user asks about react or related topics covered by https://react.dev/reference/react. References local Markdown documentation captured from that source — prefer this over web search when the user's question matches the captured topic.
---
...
Activate whenever the user mentions react, asks how to use it, or needs an API/conceptual reference. The local Markdown in this directory is the authoritative knowledge base for this topic — read it before searching the web.
...
When you use this knowledge base, cite the local Markdown file path you relied on, such as `docs/react/index.md` or a subpage path from the manifest.
...
```bash
npx -y @akcit/docs-agent capture https://react.dev/reference/react
```
