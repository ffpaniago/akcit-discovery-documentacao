---
title: "reusing-logic-with-custom-hooks.md"
source: https://react.dev/learn/reusing-logic-with-custom-hooks.md
captured_at: 2026-09-06T13:01:57.596Z
---
...
If your linter is [configured for React,](learn/editor-setup.md) it will enforce this naming convention. Scroll up to the sandbox above and rename `useOnlineStatus` to `getOnlineStatus`. Notice that the linter won't allow you to call `useState` or `useEffect` inside of it anymore. Only Hooks and components can call other Hooks!
...
When you change `serverUrl` or `roomId`, the Effect ["reacts" to your changes](learn/lifecycle-of-reactive-effects.md) and re-synchronizes. You can tell by the console messages that the chat re-connects every time that you change your Effect's dependencies.
...
Adding a dependency on `onReceiveMessage` is not ideal because it will cause the chat to re-connect every time the component re-renders. [Wrap this event handler into an Effect Event to remove it from the dependencies:](learn/removing-effect-dependencies.md)
...
Although this code is quite repetitive, [it's correct to keep these Effects separate from each other.](learn/removing-effect-dependencies.md) They synchronize two different things, so you shouldn't merge them into one Effect. Instead, you can simplify the `ShippingForm` component above by extracting the common logic between them into your own `useData` Hook:
...
Today, with the [`use`](reference/react/use.md) API, data can be read in render by passing a [Promise](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Promise) to `use`:
...
Inside `useInterval`, wrap the tick callback into an Effect Event, as you did [earlier on this page.](learn/reusing-logic-with-custom-hooks.md)
