---
title: "sharing-state-between-components.md"
source: https://react.dev/learn/sharing-state-between-components.md
captured_at: 2026-09-06T13:01:48.511Z
---
...
Clicking the "Show" button in either `Panel` needs to change the active index in `Accordion`. A `Panel` can't set the `activeIndex` state directly because it's defined inside the `Accordion`. The `Accordion` component needs to *explicitly allow* the `Panel` component to change its state by [passing an event handler down as a prop](learn/responding-to-events.md):
