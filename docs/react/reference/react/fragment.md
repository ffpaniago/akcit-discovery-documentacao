...
captured_at: 2026-09-06T13:02:13.511Z
---
...
- **optional** `key`: Fragments declared with the explicit `<Fragment>` syntax may have [keys.](learn/rendering-lists.md)
- <CanaryBadge /> **optional** `ref`: A ref object (e.g. from [`useRef`](reference/react/useref.md)) or [callback function](reference/react-dom/components/common.md). React provides a `FragmentInstance` as the ref value that implements methods for interacting with the DOM nodes wrapped by the Fragment.
...
Fragment `ref`s let you add event listeners to a group of elements without adding a wrapper DOM node. Use a [ref callback](reference/react-dom/components/common.md) to attach and clean up listeners:
