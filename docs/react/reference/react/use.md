---
title: "use.md"
source: https://react.dev/reference/react/use.md
captured_at: 2026-09-06T13:02:25.108Z
---


<Intro>

`use` is a React API that lets you read a resource during rendering, such as a [Promise](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Promise) or [context](learn/passing-data-deeply-with-context.md).
...
* When passing a Promise from a Server Component to a Client Component, its resolved value must be [serializable](reference/rsc/use-client.md).

---

### <CanaryBadge /> `use(browser())` {/*use-browser*/}

Call `use` with the value returned by [`browser`](reference/react-dom/browser.md) in a component that should only render in the browser:
...
During server rendering, the component calling `use(browser())` suspends and React includes the closest [`<Suspense>`](reference/react/suspense.md) boundary's fallback in the HTML. In the browser, `use(browser())` returns `undefined`, so the component renders normally.

[See an example below.](#rendering-a-component-only-in-the-browser)

#### Parameters {/*browser-parameters*/}

* `browserValue`: The value returned by [`browser`](reference/react-dom/browser.md).
...
* In a React Server Components app, `use(browser())` must be called from a [Client Component](reference/rsc/use-client.md), not a [Server Component](reference/rsc/server-components.md).
...
Reading the value requires two `use` calls because the context value itself isn't awaited. See [Before you use context](learn/passing-data-deeply-with-context.md) for alternatives to consider before reaching for context.
...
In this example, `Albums` calls `use` with a cached Promise. The component suspends while the Promise is pending, and React displays the nearest Suspense fallback. Rejected Promises propagate to the nearest [Error Boundary](reference/react/component.md).
...
Instead, pass a Promise from a cache, a [Suspense-enabled framework](reference/react/suspense.md), or a Server Component:
...
[React doesn't preserve state for renders that suspended before mounting](reference/react/suspense.md). After each suspension, React retries rendering from scratch, so any Promise created during render is recreated.
...
The way you cache Promises depends on the framework you use with Suspense. Frameworks typically provide built-in caching mechanisms. If you don't use a framework, you can use a simple module-level cache like the one above, or a [Suspense-enabled data source](reference/react/suspense.md).
...
See [Revealing content together at once](reference/react/suspense.md) for guidance on where to place Suspense boundaries.
...
If the Promise passed to `use` is rejected, the error propagates to the nearest [Error Boundary](reference/react/component.md). Wrap the component that calls `use` in an Error Boundary to display a fallback when the Promise is rejected.
...
Pass the value returned by [`browser`](reference/react-dom/browser.md) to `use` inside a component that should only render in the browser.
