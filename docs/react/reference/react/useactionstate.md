---
title: "useActionState.md"
source: https://react.dev/reference/react/useActionState.md
captured_at: 2026-09-06T13:01:59.834Z
---


<Intro>

`useActionState` is a React Hook that lets you update state with side effects using [Actions](reference/react/usetransition.md).
...
2. A `dispatchAction` function that you call inside [Actions](reference/react/usetransition.md).
...
* When using Server Functions, `initialState` needs to be [serializable](reference/rsc/use-server.md) (values like plain objects, arrays, strings, and numbers).
* If `dispatchAction` throws an error, React cancels all queued actions and shows the nearest [Error Boundary](reference/react/component.md).
...
You can wrap it in [`startTransition`](reference/react/starttransition.md), or pass it to an [Action prop](reference/react/usetransition.md). Calls outside that scope won’t be treated as part of the Transition and [log an error](#async-function-outside-transition) on development mode.
...
* If you set state after `await` in the `reducerAction` you currently need to wrap the state update in an additional `startTransition`. See the [startTransition](reference/react/usetransition.md) docs for more info.
* When using Server Functions, `actionPayload` needs to be [serializable](reference/rsc/use-server.md) (values like plain objects, arrays, strings, and numbers).
...
You can typically solve this by [using with useOptimistic](reference/react/useactionstate.md) but for more complex cases you may want to consider [cancelling queued actions](#cancelling-queued-actions) or not using `useActionState`.
...
When you pass the `dispatchAction` function to a component that exposes an [Action prop](reference/react/usetransition.md), you don't need to call `startTransition` or `useOptimistic` yourself.
...
See the [`<form>`](reference/react-dom/components/form.md) docs for more information on using Actions with forms.
...
For unknown errors, such as `undefined is not a function`, you can throw an error. React will cancel all queued Actions and shows the nearest [Error Boundary](reference/react/component.md) by rethrowing the error from the `useActionState` hook.
