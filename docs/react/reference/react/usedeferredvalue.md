---
title: "useDeferredValue.md"
source: https://react.dev/reference/react/useDeferredValue.md
captured_at: 2026-09-06T13:02:02.161Z
---
...
This example assumes you use a data source that [activates a Suspense boundary](reference/react/suspense.md), such as a Promise you read with [`use`](reference/react/use.md).

[Learn more about Suspense.](reference/react/suspense.md)

</Note>


In this example, the `SearchResults` component [suspends](reference/react/suspense.md) while fetching the search results. Try typing `"a"`, waiting for the results, and then editing it to `"ab"`. The results for `"a"` get replaced by the loading fallback.
...
First, optimize `SlowList` to skip re-rendering when its props are the same. To do this, [wrap it in `memo`:](reference/react/memo.md)
