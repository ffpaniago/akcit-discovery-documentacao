---
title: "use-client.md"
source: https://react.dev/reference/rsc/use-client.md
captured_at: 2026-09-06T13:03:24.772Z
---
...
When a file marked with `'use client'` is imported from a Server Component, [compatible bundlers](learn/creating-a-react-app.md) will treat the module import as a boundary between server-run and client-run code.
...
In a React app, components are often split into separate files, or [modules](learn/importing-and-exporting-components.md).

For apps that use React Server Components, the app is server-rendered by default. `'use client'` introduces a server-client boundary in the [module dependency tree](learn/understanding-your-ui-as-a-tree.md), effectively creating a subtree of Client modules.
...
During render, the framework will server-render the root component and continue through the [render tree](learn/understanding-your-ui-as-a-tree.md), opting-out of evaluating any code imported from client-marked code.
...
Client Components can render Server Components because you can pass JSX as props. In this case, `InspirationGenerator` receives `Copyright` as [children](learn/passing-props-to-a-component.md). However, the `InspirationGenerator` module never directly imports the `Copyright` module nor calls the component, all of that is done by `App`. In fact, the `Copyright` component is fully executed before `InspirationGenerator` starts rendering.
