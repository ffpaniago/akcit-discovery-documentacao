---
title: "components-and-hooks-must-be-pure.md"
source: https://react.dev/reference/rules/components-and-hooks-must-be-pure.md
captured_at: 2026-09-06T13:03:20.424Z
---
...
* **Idempotent** – You [always get the same result every time](learn/keeping-components-pure.md) you run it with the same inputs – props, state, context for component inputs; and arguments for hook inputs.
...
[Side effects](learn/keeping-components-pure.md) should not run [in render](#how-does-react-run-your-code), as React can render components multiple times to create the best possible user experience.
...
While render must be kept pure, side effects are necessary at some point in order for your app to do anything interesting, like showing something on the screen! The key point of this rule is that side effects should not run [in render](#how-does-react-run-your-code), as React can render components multiple times. In most cases, you'll use [event handlers](reference/learn/responding-to-events.md) to handle side effects. Using an event handler explicitly tells React that this code doesn't need to run during render, keeping render pure. If you've exhausted all options – and only as a last resort – you can also handle side effects using `useEffect`.
...
A component's props and state are immutable [snapshots](reference/learn/state-as-a-snapshot.md). Never mutate them directly. Instead, pass new props down, and use the setter function from `useState`.
