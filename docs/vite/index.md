...
source: https://vite.dev/llms-full.txt
captured_at: 2026-09-06T13:04:13.285Z
---
...
HTML files stand [front-and-center](guide.md) of a Vite project, serving as the entry points for your application, making it simple to build single-page and [multi-page applications](guide/build-html.md).
...
You can also use the `query` option to provide queries to imports, for example, to import assets [as a string](guide/assets-html.md) or [as a url](guide/assets-html.md):
...
| `--profile`               | Start built-in Node.js inspector (check [Performance bottlenecks](guide/troubleshooting.md))                                                                    |
...
The CLI commands `$ vite dev` and `$ vite preview` can also be used for SSR apps. You can add your SSR middlewares to the development server with [`configureServer`](guide/api-plugin.md) and to the preview server with [`configurePreviewServer`](guide/api-plugin.md).
...
Vite strives to offer established patterns out of the box, so before creating a new plugin make sure that you check the [Features guide](guide/features.md) to see if your need is covered. Also review available community plugins, both in the form of a [compatible Rollup plugin](https://github.com/rollup/awesome) and [Vite Specific plugins](https://github.com/vitejs/awesome-vite#plugins).
...
All rolldown hooks are [per-environment hooks](guide/api-environment-plugins.md).
...
These hooks also have an extended `options` parameter with additional Vite-specific properties. You can read more in the [SSR documentation](guide/ssr.md).
...
* **Scope:** [Global](guide/api-environment-plugins.md)
...
* **Scope:** [Global](guide/api-environment-plugins.md)
  ...
  Same as [`configureServer`](guide/api-plugin-html.md) but for the preview server. Similarly to `configureServer`, the `configurePreviewServer` hook is called before other middlewares are installed. If you want to inject a middleware **after** other middlewares, you can return a function from `configurePreviewServer`, which will be called after internal middlewares are installed:
...
* **Kind:** `async`, `sequential`
* **Scope:** [Per-environment](guide/api-environment-plugins.md)
...
On the client side, use [`hot.on`](guide/api-hmr-html.md) to listen to the events:
...
To send events from the client to the server, we can use [`hot.send`](guide/api-hmr-html.md):
...
See [Client-server Communication](guide/api-plugin-html.md) for more details, including a section on [Typing Custom Events](guide/api-plugin-html.md).
...
Pre-processes `.css`, `.scss`, `.sass`, `.less`, `.styl` and `.stylus` files to plain CSS so it can be used in browsers or parsed by other tools. Similar to the [built-in CSS pre-processing support](guide/features.md), the corresponding pre-processor must be installed if used.
...
When running `vite` from the command line, Vite will automatically try to resolve a config file named `vite.config.js` inside [project root](guide.md) (other JS and TS extensions are also supported).
...
If the config needs to conditionally determine options based on the command (`serve` or `build`), the [mode](guide/env-and-mode.md) being used, if it's an SSR build (`isSsrBuild`), or is previewing the build (`isPreview`), it can export a function instead:
...
It is important to note that in Vite's API the `command` value is `serve` during dev (in the cli [`vite`](guide/cli.md), `vite dev`, and `vite serve` are aliases), and `build` when building for production ([`vite build`](guide/cli.md)).
...
`ExternalFetchResult` instructs the module runner to import the module using the `runExternalModule` method on the [`ModuleEvaluator`](guide/api-environment-runtimes.md). In this case, the default module evaluator will use the runtime's native `import` instead of processing the file through Vite.
...
`environment.hot` allows plugins to communicate with the code on the application side for a given environment. This is the equivalent of [the Client-server Communication feature](guide/api-plugin.md), but supports environments other than the client environment.
...
Given a Vite server configured in middleware mode as described by the [SSR setup guide](guide/ssr.md), let's implement the SSR middleware using the environment API. Remember that it doesn't have to be called `ssr`, so we'll name it `server` in this example. Error handling is omitted.
...
There are [multiple communication levels for the `DevEnvironment`](guide/api-environment-frameworks.md). To make it easier for frameworks to write runtime agnostic code, we recommend to implement the most flexible communication level possible.
...
See [Project Root](guide.md) for more details.
...
See [Public Base Path](guide/build.md) for more details.
...
See [Env Variables and Modes](guide/env-and-mode.md) for more details.
...
Array of plugins to use. Falsy plugins are ignored and arrays of plugins are flattened. If a promise is returned, it would be resolved before running. See [Plugin API](guide/api-plugin.md) for more details on Vite plugins.
...
See [The `public` Directory](guide/assets.md) for more details.
...
More advanced custom resolution can be achieved through [plugins](guide/api-plugin.md).

::: warning Using with SSR
If you have configured aliases for [SSR externalized dependencies](guide/ssr.md), you may want to alias the actual `node_modules` packages. Both [Yarn](https://classic.yarnpkg.com/en/docs/cli/add/#toc-yarn-add-alias) and [pnpm](https://pnpm.io/aliases/) support aliasing via the `npm:` prefix.
:::
...
Enables the tsconfig paths resolution feature. `paths` option in `tsconfig.json` will be used to resolve imports. See [Features](guide/features.md) for more details.
...
* **Type:** `string`
* **Related:** [Content Security Policy (CSP)](guide/features.md)
...
This option doesn't have any effect when using [Lightning CSS](guide/features.md). If enabled, [`css.lightningcss.cssModules`](https://lightningcss.dev/css-modules.html) should be used instead.
...
The search is done using [postcss-load-config](https://github.com/postcss/postcss-load-config) and only the supported config file names are loaded. Config files outside the workspace root (or the [project root](guide.md) if no workspace is found) are not searched by default. You can specify a custom path outside of the root to load the specific config file instead if needed.
...
Selects the engine used for CSS processing. Check out [Lightning CSS](guide/features.md) for more information.
...
* **Related:** [Static Asset Handling](guide/assets.md)
...
See [here](guide/env-and-mode.md) for more about environment files.
...
Whether your application is a Single Page Application (SPA), a [Multi Page Application (MPA)](guide/build.md), or Custom Application (SSR and frameworks with custom HTML handling):
...
Learn more in Vite's [SSR guide](guide/ssr.md). Related: [`server.middlewareMode`](./server-options#server-middlewaremode).
...
In some cases, you might also want to configure the underlying dev server (e.g. to add custom middlewares to the internal [connect](https://github.com/senchalabs/connect) app). In order to do that, you need to write your own [plugin](guide/using-plugins-html.md) and use [configureServer](guide/api-plugin-html.md) function.
...
* **Related:** [Warm Up Frequently Used Files](guide/performance-html.md)
...
If set to `null`, no files will be watched. [`server.watcher`](guide/api-javascript-html.md) will provide a compatible event emitter, but calling `add` or `unwatch` will have no effect.
...
* **Related:** [appType](./shared-options#apptype), [SSR - Setting Up the Dev Server](guide/ssr.md)
...
Vite will search for the root of the potential workspace and use it as default. A valid workspace met the following conditions, otherwise will fall back to the [project root](guide.md).
...
Accepts a path to specify the custom workspace root. Could be a absolute path or a path relative to [project root](guide.md). For example:
...
This blocklist does not apply to [the public directory](guide/assets.md). All files in the public directory are served without any filtering, since they are copied directly to the output directory during build.
...
* **Related:** [Browser Compatibility](guide/build.md)
...
Note: the polyfill does **not** apply to [Library Mode](guide/build.md). If you need to support browsers without native dynamic import, you should probably avoid using it in your library.
...
The resolved dependency paths can be further modified using [`experimental.renderBuiltUrl`](guide/build.md).
...
Specify the output directory (relative to [project root](guide.md)).
...
Specify the directory to nest generated assets under (relative to `build.outDir`. This is not used in [Library Mode](guide/build.md)).
...
* **Default:** `false`
* **Experimental**
* **Related:** [Chunk Import Map Optimization](guide/features.md)
...
* **Related:** [Dynamic Import](guide/features.md)
...
* **Related:** [Library Mode](guide/build.md)
...
* **Default:** `false`
* **Related:** [License](guide/features.md)
...
* **Type:** `boolean | string`
* **Default:** `false`
* **Related:** [Backend Integration](guide/backend-integration.md)
...
If you are writing a plugin and need to inspect each output chunk or asset's related CSS and static assets during the build, you can also use [`viteMetadata` output bundle metadata API](guide/api-plugin.md).

## build.ssrManifest

* **Type:** `boolean | string`
* **Default:** `false`
* **Related:** [Server-Side Rendering](guide/ssr.md)
...
Set to `false` to disable writing the bundle to disk. This is mostly used in [programmatic `build()` calls](guide/api-javascript.md) where further post processing of the bundle is needed before writing to disk.
...
* **Related:** [Dependency Pre-Bundling](guide/dep-pre-bundling.md)
...
* **Type:** `string[] | true`
* **Related:** [SSR Externals](guide/ssr.md)
...
For a more robust long term implementation, the plugin hook should handle for [multiple environments](guide/api-environment-plugins-html.md) using fine-grained environment options instead of relying on the environment name.
...
We're planning to deprecate the `handleHotUpdate` plugin hook in favor of [`hotUpdate` hook](guide/api-environment-plugins.md) to be [Environment API](guide/api-environment.md) aware, and handle additional watch events with `create` and `delete`.
...
The [`handleHotUpdate` hook](guide/api-plugin.md) allows to perform custom HMR update handling. A list of modules to be updated is passed in the `HmrContext`.
...
* `server.moduleGraph` -> [`environment.moduleGraph`](guide/api-environment-instances.md)
...
`server.ssrLoadModule` has been replaced by importing from a [Module Runner](guide/api-environment-runtimes.md).
...
See [Shared plugins during build](guide/api-environment-plugins.md).
...
Check out [Using Plugins](guide/using-plugins.md) for information on how to use plugins.
...
Vite supports [React Server Components (RSC)](https://react.dev/reference/rsc/server-components) through the plugin. It utilizes the [Environment API](guide/api-environment.md) to provide low-level primitives that React frameworks can use to integrate RSC features. You can try a minimal standalone RSC application with:
...
[Vite plugins](guide/api-plugin.md) are an extension of Rollup's plugin interface. Check out the [Rollup Plugin Compatibility section](guide/api-plugin.md) for more information.
...
We periodically deprecate features that have been superseded by better alternatives in Minor releases. Deprecated features will continue to work with a type or logged warning. They will be removed in the next major release after entering deprecated status. The [Migration Guide](guide/migration-html.md) for each major will list these removals and document an upgrade path for them.
